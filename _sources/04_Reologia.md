...
La reología es el estudio de cómo los materiales se deforman bajo un esfuerzo (o estrés). Por lo tanto, los *modelos reológicos* a menudo se expresan como relaciones simples entre esfuerzo y deformación. Los modelos hidráulicos estándar ya asumen un modelo reológico para las simulaciones hidrodinámicas. Asumen que el agua comienza a deformarse (movimiento o deformación) bajo cualquier esfuerzo (intersección en cero en la relación esfuerzo-deformación), la deformación aumenta linealmente con el esfuerzo, y la viscosidad del agua es la relación entre el esfuerzo y la deformación. Estas son las suposiciones del flujo "newtoniano". Existen fluidos que se desvían de estas suposiciones, incluyendo una intersección esfuerzo-deformación distinta de cero o una relación esfuerzo-deformación no lineal, o ambas.
...
### Flujo viscoso
Dominado por la fricción interna del fluido intersticial, que resiste el movimiento mediante una viscosidad dinámica constante. Como mecanismo dominante, la resistencia es proporcional a la tasa de deformación ($\dot{\gamma}$). No existe un esfuerzo de fluencia: el flujo comienza a moverse con cualquier esfuerzo, por pequeño que sea, y la resistencia aumenta linealmente con la velocidad de deformación. Esta condición es típica de flujos diluidos, donde la mezcla está compuesta por agua con bajo contenido de sólidos o por suspensiones finas con comportamiento newtoniano o cuasi-newtoniano. Cuando la resistencia viscosa domina, se emplean modelos newtonianos o generalizados lineales, donde el esfuerzo cortante es directamente proporcional a la tasa de cizalla.

### Flujo plástico
Dominado por la cohesión y viscosidad interna de una matriz de sedimentos finos. Como mecanismo dominante se refiere a un esfuerzo de fluencia ($\tau_y$): el flujo no se moviliza hasta superar ese umbral. Luego, se comporta como un fluido viscoso. La resistencia viene de la cohesión de finos (arcillas, limos) y la viscosidad del fluido. Aparece en flujos hiperconcentrados, ricos en finos cohesivos.

### Flujo turbulento
La resistencia al flujo proviene principalmente de la fase fluida (la mezcla de agua y sedimentos finos). Se generan remolinos caóticos y un intenso intercambio de momento dentro del propio fluido. Cuando los esfuerzos turbulentos son dominantes, se suelen usar modelos que incluyen explícitamente un término de esfuerzo cortante turbulento, que a menudo es proporcional al cuadrado de la velocidad o del gradiente de velocidad (tasa de deformación). Esta condición es típica de los flujos de menor concentración de sólidos, como los flujos hiperconcentrados o los flujos de lodo (*mudflows*)

### Flujo dispersivo
Dominado por colisiones entre partículas sólidas grandes. Como mecanismo dominante está que la energía se disipa por interacciones granulares, especialmente colisiones dispersivas entre bloques o gravas suspendidas. A medida que los granos chocan, se rozan y se empujan unos a otros, se genera una presión dispersiva que se opone al cizallamiento. Este es el famoso efecto Bagnold. Es un comportamiento no viscoso, sino granular. Cuando dominan los esfuerzos dispersivos, los modelos basados en la fricción de Coulomb o los modelos de fluidos dilatantes de Bagnold son más apropiados. El término de esfuerzo dispersivo suele estar relacionado con el cuadrado del tamaño del grano y el cuadrado de la tasa de cizalla.

### Flujo dilatante
Dominado por la expansión volumétrica del material granular cuando se somete a cizallamiento. Como mecanismo dominante, durante la deformación, las partículas necesitan separarse unas de otras para poder reorganizarse, lo que genera un aumento del volumen total (dilatancia) y una presión normal adicional que incrementa la resistencia al cizallamiento. Este fenómeno ocurre especialmente en materiales granulares densamente empaquetados, donde el movimiento implica vencer tanto la fricción como el trabajo mecánico necesario para expandir el volumen del material. Aunque puede coexistir con la fricción y la dispersión, el flujo dilatante refleja un comportamiento típico de materiales granulares densos que expanden su estructura al fluir, como ciertos debris flows o avalanchas densas que muestran un aumento temporal de la resistencia al inicio del movimiento.
...
| **Coulombiano**          | $\tau = \sigma_n , \tan \phi + c$                                                                                   | $\sigma_n$: presión normal <br> $\phi$: ángulo de fricción interna <br> $c$: cohesión        |
...
El modelo de Bingham solo requiere dos datos de entrada por parte del usuario: el esfuerzo de cedencia (el intercepto de la relación esfuerzo-deformación) y la viscosidad del flujo cargado de sedimento (la pendiente de la relación esfuerzo-deformación). en el modelo HEC RAS Las opciones para estos se describen en las secciones de Esfuerzo de Cedencia y Viscosidad a continuación de a siguiente forma.
...
La ecuación para la longitud de mezcla de Prandtl se define como:


$$l_m = kz$$  


donde: $k$ = el coeficiente de Von Kármán ($\cong 0.41$), $z$ = la distancia proporcional desde el contorno (lecho).
...
**Supuestos del modelo**: El modelo, basado en el centro de masa, asume que el movimiento está controlado principalmente por un coeficiente de fricción por deslizamiento $\mu$, y una relación masa-resistencia aerodinámica $M/D$. El parámetro $\mu$ domina el comportamiento de la velocidad en la zona de alcance (*runout*).

La velocidad puede calcularse a lo largo de segmentos de pendiente más o menos homogéneos (Gamma, 2000), o  celda por celda en el camino del flujo (Horton et al., 2013; Wichmann, 2017).

**Cálculo de la velocidad**: La velocidad $v_i$ en una celda se calcula según:
...
**Calibración de la distancia de alcance**: La distancia de alcance del flujo se calibra ajustando los dos parámetros de fricción.  Sin embargo, como diferentes combinaciones de $\mu$ y $M/D$ pueden producir la misma distancia de alcance en la práctica, se mantiene $M/D$ constante a lo largo del trayecto, calibrándolo una sola vez para alcanzar rangos realistas de velocidad máxima.
...
### Tabla 6.5 Lista de modelos numéricos seleccionados para el cálculo del alcance de deslizamientos (adaptado de Kang y Chan (2018b))

Tabla tomada de Kang et al. 
